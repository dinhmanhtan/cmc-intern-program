...
func main() {
	...
	exportHandler := handler.NewExportHandler(assetService, scanService) // NEW - Bài 6
	...
	// Export endpoints (Bài 6 - Day 3 Bonus!)
	mux.HandleFunc("GET /assets/export", exportHandler.ExportAssets)              // Export all assets (CSV/JSON)
	mux.HandleFunc("GET /assets/{id}/results/export", exportHandler.ExportAssetResults) // Export 1 asset results
	...
	// Wrap mux with CORS middleware (Bài 3 - Day 3: Frontend integration)
	...
	log.Println("   ✓ IP Scan - Geolocation & ASN lookup (NEW - Day 3)")
	log.Println("   ✓ Port Scan - TCP port scanning on private IPs (NEW - Day 3)")
	log.Println("   ✓ CORS Enabled - Frontend can connect (NEW - Day 3)")
