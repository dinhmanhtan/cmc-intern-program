...
type ScanService struct {
	...
	ipScanner        *scanner.IPScanner       // NEW: IP geolocation (Day 3 - Bài 1)
	portScanner      *scanner.PortScannerImpl // NEW: Port scanning (Day 3 - Bài 1)

	// In-memory results cache for new scan types (IP, Port)
	// Key: scan job ID
	ipResultsMu  sync.RWMutex
	ipResults    map[string]*model.IPScanResult
	portResultsMu sync.RWMutex
...
func NewScanService(store storage.Storage, scanStore storage.ScanStorage) (*ScanService, error) {
	...
	return &ScanService{
		...
		portScanner:      scanner.NewPortScannerImpl(),
		ipResults:        make(map[string]*model.IPScanResult),
...
func (s *ScanService) performScan(asset *model.Asset, job *model.ScanJob) {
	...
	case model.ScanTypeIP: // NEW - Bài 1 Day 3
		err = s.performIPScan(asset, job)
	case model.ScanTypePort: // NEW - Bài 1 Day 3
...
// ============================================================================
// NEW SCAN METHODS - Day 3, Bài 1
// ============================================================================

// performIPScan executes IP geolocation and ASN lookup
func (s *ScanService) performIPScan(asset *model.Asset, job *model.ScanJob) error {
	...
	// Set IDs and store result in memory cache
	...
	s.ipResultsMu.Lock()
	s.ipResults[job.ID] = result
	s.ipResultsMu.Unlock()

	job.Results = 1
	return nil
}

// performPortScan executes TCP port scanning (only safe on private IPs)
func (s *ScanService) performPortScan(asset *model.Asset, job *model.ScanJob) error {
	...
	s.portResultsMu.Lock()
	s.portResults[job.ID] = result
	s.portResultsMu.Unlock()

	job.Results = len(result.OpenPorts)
	if job.Results == 0 {
		job.Results = 1 // mark as having result even if no open ports
	}
	return nil
}
...
func (s *ScanService) GetScanResults(jobID string) (interface{}, error) {
	...
	case model.ScanTypeIP: // NEW - Bài 1 Day 3
		s.ipResultsMu.RLock()
		res, ok := s.ipResults[jobID]
		s.ipResultsMu.RUnlock()
		if !ok {
			return nil, fmt.Errorf("IP scan result not found for job: %s", jobID)
		}
		return map[string]interface{}{
			"job_id":    jobID,
			"scan_type": "ip",
			"results":   []*model.IPScanResult{res},
		}, nil
	case model.ScanTypePort: // NEW - Bài 1 Day 3
		s.portResultsMu.RLock()
		res, ok := s.portResults[jobID]
		s.portResultsMu.RUnlock()
		if !ok {
			return nil, fmt.Errorf("port scan result not found for job: %s", jobID)
		}
		return map[string]interface{}{
			"job_id":    jobID,
			"scan_type": "port",
			"results":   []*model.PortScanResult{res},
		}, nil
	default:
